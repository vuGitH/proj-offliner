 ...
 */
function t() {
  //My Drive/Work/Aquapowder/eMailsMarkUp
  ...
  var domain = test.url;
  response = UrlFetchApp.fetch(domain).getContentText('windows-1251');
  response.replace(/href\=\"\//gi, 'href="' + domain + '/').
           replace(/src\=\"\//gi, 'src="' + domain + '/');

  txtRep += '\n' + response + '\n';
  //txtRep+='\n'+response.getContentText('windows-1251')+'\n';
  var rep = folder.createFile("testReport_" + 
                              Utilities.formatDate(time, tZone, 'ddMMYY_HH:mm:ss'),
                              txtRep, MimeType.PLAIN_TEXT);
}