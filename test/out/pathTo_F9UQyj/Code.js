...
var webAppUrl=ScriptApp.getService().getUrl();   // --- URL of Web-app https://script.google.com/macros/s/AKfycbw1iBd-dytnWD2TyY_B6tSblxYz5GFJKdiHk8qNhkVdQ24MHgkN/exec
var tZone=Session.getScriptTimeZone();
/**
 * Responds to GET request of client
 * @param{object}e - https://developers.google.com/apps-script/guides/web#url_parameters
 * @return {HtmlOutput}
 */
function doGet(e){
try{  
  ...
  // --- wAPP ---
    case modes[1]:    // mode='wApp' control page. To choose what to do
      var html=HtmlService.createHtmlOutput("<!DOCTYPE html><html><head></head><body></body></html>");
  ...
  // --- SHORT ---
    case modes[3]:      // mode='short' SMAILYC or SMAILIC or SMSlyc
      try{
        ...
        var menuPanFile=folder.createFile('jsMenuPan.js', HtmlService.createHtmlOutputFromFile('jsMenu').getContent().replace(/\<[\/]?script.*\>/gi,''), MimeType.HTML);
        Logger.log( menuPanFile.getUrl());
        return;
        */
        var html=HtmlService.createHtmlOutput("<!DOCTYPE html><html><head></head><body>");
        html.append(HtmlService.createHtmlOutputFromFile('jsLib').getContent());
        html.append( HtmlService.createHtmlOutputFromFile('jsShort').getContent() );
        html.append( HtmlService.createHtmlOutput('</body></html>').getContent() );
       return html
  ...
  // --- DEFAULT ---
    default:
      ...
      mode=modes[2];
      var html=HtmlService.createHtmlOutput('<!DOCTYPE html><html><head><base target="_blank"></head><body></body></html>');
...
//-----------------------------------------------
 ...
 var subject=subject+' '+ Utilities.formatDate(new Date(),tZone, "ddMMYY_HH:mm:ss");
...
Logger.log( 'inside getMenu mode='+mode);
  switch (mode){
    case 'wApp':
      var htmlContent=HtmlService.createTemplateFromFile('jsMenu').evaluate().getContent();
      var htmlCont=htmlContent.replace(/\<[\/]?script.*\>/g,'');
