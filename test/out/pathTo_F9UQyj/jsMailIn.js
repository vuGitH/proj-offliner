...
var Bild={
  ...
  */
  ...
  docName:"",
  docUrl:"",
  eventTypes:['click','mouseenter','mouseleave','mousedown','mouseup','mouseover','mouseout','mousemove','focus','blur','resize','edit','change','load'],
  ...
  FragmentServ:{
    ...
    setFragment:function(frgName){
       ...
       * @param{object htmlElement}parentOpt - object of parent element to which DocumentFragment appended to or inserted before
       *                                       if id undefined it's considered to be document.body of root document
       * @param{object htmlElement}beforeElOpt - optional, if is set it's an element before which the fragment is inserted to
       */
  ...
  BlockServ:{
    blNames:[],
    blIds:[],     // id could be generated while save block info on drive
    /**
     * Creates new empty block of elements and sets it name
     * @param{string}blockName - name of new block(ID)
     */
    createBlock:function(blockName,belIdsOpt,belTypesOpt,belParentsOpt,belsAttsOpt){
      var entityType='BlockServ';
      var newId=Bild.generateEntityId(enType,blockName); // id could be generated while save block info on drive
      ...
      bbs.belsAtts={};
      /**
       * Fills empty block by belements data of wich are transformed by arrays and atts-object
       * @param{object strings[]} belIds Ids of Bild-elements
       * @param{object strings[]} belTypes type of elements forming block ('DIV', 'SCRIPT', ...)
       * @param{object stringss[]} belParents parents of block's elements
       * @param{object objects{} }belsAtts - atts objects for each bel as object property ( for el with id=belIds[i] atts=belsAtts[belIds[i]]
      ...
      bbs.setBels=function(belIds,belTypes,belParents,belsAtts){
        ...
        if( ln ){
          for(var i=0;i<ln;i++){
            ...
            if( belsAtts) {
              var atts=( belsAtts[ belIds[i]]) ? belsAtts[ belIds[i]] : {id: (belTypes[i]!='BR')?belIds[i]:('br'+d.getElementsByTagName('BR').length)};
            }
     ...
     */
    fillBelsInBlock:function(blockName,belIds,belTypes,belParentIds,belAttsOpt){
      var ind=( Bild.BlockServ.blockNames.indexOf(blockName))? Bild.BlockServ.blockNames.indexOf(blockName): indexOf(Bild.BlockServ.blockNames,blockNames);
      if( ind <0){
     ...
     */
    addBelsToBlock:function(blockName,addingBelIds,addingBelTypes,addingBelParents,addingBelsAtts){
      var ind=( Bild.BlockServ.blockNames.indexOf(blockName))?Bild.BlockServ.blockNames.indexOf(blockName): indexOf(Bild.BlockServ.blockNames,blockNames);
      if( ind <0){
        var bb=Bild.BlockServ.createBlock(blockName,addingBelIds,addingBelTypes,addingBelParents,addingBelsAtts);
      }else{
        var bb=Bild.BlockServ[blockName];
        var ln=bb.belIds[blockName].length;
        if( (addingBelIds) && addingBelIds.length>0 ){
          bb.setBels(addingBelIds,addingBelTypes,addingBelParents,addingBelsAtts);
        }
      }
      return bb;
    },
     ...
     * @param{string}belId - id of element or id of bel (bilt element) belId=id
    ...
    addBelToBlock:function(blockName,belId,type,parent,atts){
      ...
      bb.addBelsToBlock(blockName,[belId],[type],[parent],{belId:atts});
      return bb[belId];
    }
  },
  /**
  ...
  ElServ:{
    ...
    * @param{object}atts - attributes 'object of named pairs - attributeName : attributevalue
    *                      atts.id= el.id , atts.style=e"...style  data..",atts.name=name, atts.clas='class1 class2...'
    *                     !- id attribute is obligatory( or strictely recommened), if not set it's set
    *                      , it's set to 'bildEl_n' where n=Bild.ElServ.els.length (index of new element)
    *                      , after attributing element to specified fragment this id would be  exchanged by id='fragmEl_n'
    *                      if atts is not defined the element object is created without attributes excluding id.
    * @param{object htmlElement}parentOpt - object of parent element.Optional. By defalut it's document.body( Bild.root.body)
    * @param{object htmlElement}beforeElOpt - object of html element before which new element should be inserted
    */
    newEl:function(type,attsOpt,parentOpt,beforeElOpt){
      ...
      var atts=attsOpt || {id:undefined};
      if( type=='BR'){
        id='br_'+(d.getElementsByTagName('BR').length-1);
      }
      var id=atts.id;
      id=( !(id) || (id==="") )?'bildEl_'+Bild.ElServ.els.length : id;
      el.id=id;Bild.ElServ.ids.push(id);
      ...
      * @param{object}parent - html element asigned to be parent of Bild element
      * @return{object}
      */
            ...
            switch(attName){
              ...
              default:
                ...
                for( var anm in bel.atts){
                  if( anm!='class' && anm!='style' && anm!='name' && anm!='id' && anm !='innerHTML'){
...
//                                           <link id="head_link0" href='https://fonts.googleapis.com/css?family=Roboto|Open+Sans|Roboto+Condensed|Merriweather|Arimo&subset=latin,cyrillic' rel='stylesheet' type='text/css'>
fm.linkGFont={type:'LINK',parent:d.head,
               atts:{id:'linkGFont',
                     href:href='https://fonts.googleapis.com/css?family=Roboto|Open+Sans|Roboto+Condensed|Merriweather|Arimo&subset=latin,cyrillic',
                     rel:'stylesheet',
                     type:'text/css'
                    }
             };
...
fm.mlPan={type:'DIV',parent:d.body,
            atts:
            {id:'mlPan',
             style:"width:95%;height:80%;min-width:300px;min-height:200px,padding:10px;border:solid 1px #ccc;"
            +"background-color:whitesmoke;border-radius:30px;box-shadow:0px 4px 8px #aaa;position:relative;",
             class:'mlPan'
             }
           };
var o=fm.mlPan;           
var mlPan=BE.newEl(o.type,
                   o.atts,
                   o.parent).el;
// Fields
/* Any field are drawn as pair: lable : field. Each - Lable and Field is included in pad-div-element.
   any pad is inline-block, so that in the case of narrow screen lable and fiels place in different lines
   one after another. While space is enough:   Right edge of lables pads are aligned. Left edge of fields' pads are aligned
...
*/
var contEl='frmPan';
var elm= ['frmPan','mlForm','padLN1','labN1'      ,'padIN1','inpN1' ,'br'    ,'padLN2','labN2'   ,'padIN2','inpN2' ,'br'    ,'padLN3','labN3'       ,'padIN3','inpN3' ,'br'    ,'padLFrom','labFrom' ,'padSelFrom','selFrom' ,'br'    ,'padLTo','labTo' ,'padITo','inpTo' ,'br'    ,'padLSubj','labSubj' ,'padISubj','inpSubj' ,'br'    ,'padLAlarm','labAlarm'  ,'padAlarm','alarm'   ,'br'    ,'padLMess','labMess' ,'br'    ,'padTArMess','tArMess'  ,'panButts','buttPreview','buttSend'];
var type=['DIV'    ,'FORM' ,'DIV'   ,'SPAN'       ,'DIV'   ,'INPUT' ,'BR'    ,'DIV'   ,'SPAN'    ,'DIV'   ,'INPUT' ,'BR'    ,'DIV'   ,'SPAN'        ,'DIV'   ,'INPUT' ,'BR'    ,'DIV'     ,'SPAN'    ,'DIV'     ,'SELECT'    ,'BR'    ,'DIV'   ,'SPAN'  ,'DIV'   ,'INPUT' ,'BR'    ,'DIV'     ,'SPAN'    ,'DIV'     ,'INPUT'   ,'BR'    ,'DIV'      ,'SPAN'      ,'DIV'     ,'DIV'     ,'BR'    ,'DIV'     ,'SPAN'    ,'BR'    ,'DIV'       ,'TEXTAREA'  ,'DIV'     ,'BUTTON'     ,'BUTTON'  ];
var innT=[''       ,''     ,''      ,'First Name:',''      ,''      ,''      ,''      ,'Surname:',''      ,''      ,''      ,''      ,'Family Name:',''      ,''      ,''      ,''        ,'From:'   ,''        ,''          ,''      ,''      ,'To:'   ,''      ,''      ,''      ,''        ,'Subject:',''        ,''        ,''      ,''         ,'Внимание!',''        ,''        ,''      ,''        ,'Message:',''      ,''          ,''          ,''        ,'Preview'    ,'Send'    ];
var prnt=['mlPan'  ,contEl ,contEl  ,'padLN1'     ,contEl  ,'padIN1',contEl  ,contEl  ,'padLN2'  ,contEl  ,'padIN2',contEl  ,contEl  ,'padLN3'      ,contEl  ,'padIN3',contEl  ,contEl    ,'padLFrom',contEl    ,'padSelFrom',contEl  ,contEl  ,'padLTo',contEl  ,'padITo',contEl  ,contEl    ,'padLSubj',contEl    ,'padISubj',contEl  ,contEl    ,'padLAlarm'  ,contEl    ,'padAlarm',contEl  ,contEl    ,'padLMess',contEl  ,contEl      ,'padTArMess',contEl    ,'panButts'   ,'panButts']; 
/**
...
/**
 * Creates elements of fragment without attributes data on the basis of of arrays
 * elm,type,innT,prnt
...
/*-- form panel --*/
formBels.frmPan.atts={id:'frmPan',
                      style:"width:95%;height:80%;min-width:300px;min-width:200px;position:relative;padding:10px;border:solid 1px #afa;"
                           +"background-color:whitesmoke;border-radius:30px;box-shadow:0px 4px 8px #aaa;"
                           +"margin-top:5px;margin-left:auto;margin-right:auto;"};
/*-- mailer Form --*/
formBels.mlForm.atts={id:'mlForm',
...
/* -- Name block --*/
    ...
    var style0=BE.newEl( 'STYLE',{id:'head_stl'+nStyle},d.head).el;
    // CSS cLASSES 
    var classes=""
    +".lable-pad{min-width:150px;min-height:20px;padding:2px;text-align:right;display:inline-block;margin-right:10px;background-color:#fff;}"                     
    +".lable{width:100%;height:100%;text-align:right;font-size:14px;font-family:Arial,sans-serif;color:#666;display:inline-block;}"
    +".inp-pad{min-width:300px;min-height:20px;margin-left:10px;text-align:left;display:inline-block;background-color:wheat;}"
    +".inp-txt{width:100%;height:100%;color:#444;font-family:Arial,sans-serif;text-align:left;background-color:white;border:solid 0px #aaa;}"
    +".tar-messpad{min-width:350px;min-height:200px;width:90%;overflow:visible;margin:10px auto;padding:15px;border-radius:14px;text-align:left;display:block;background-color:#FFECB8;}"
    +".tar-mess{width:100%;min-height:180px;color:#444;font-family:Arial,sans-serif;text-align:left;background-color:white;border:solid 0px #aaa;border-radius:6px;margin:0 auto;}"
    +".button{color:#044;font-size:12px;font-family:Arial,san-serif;background-color:#fff;"
              +"width:60px;height:22px;text-align:center;border:solid 2px #5cafaf;border-radius:10px;"
              +"margin:4px;padding:0px;}"
    +".alarm,#alarm{width:100%;min-height:20px;color:red;font-family:Arial,sans-serif;text-align:center;background-color:#ffa;border:double 3px red;border-radius:6px;}"
    +".alarm-pad{min-width:150px;min-height:20px;width:70%;margin-left:20px;display:inline-block;background-color:white;text-align:left;}"
    +".lable-alarm{width:100%;height:100%;text-align:right;font-size:14px;font-family:Arial,sans-serif;color:#f00;display:inline-block;}";
    //css classes:
...
var sizePar=(suff!='From')?'26':'1';
  /* -- lable pad -- */
  formBels['padL'+suff].atts={id:'padL'+suff,
                        class:'lable-pad'};
  formBels['lab'+suff].atts={id:'lab'+suff,
                       class:'lable',
                       innerHTML:innT[indexOf(elm,'lab'+suff)]};
  /* -- input pad -- */
  formBels['pad'+prefx1+suff].atts={id:'pad'+prefx1+suff,
               class:'inp-pad'
               };
  formBels[prefx2+suff].atts={id:prefx2+suff,
                class:'inp-txt',
                required:true,
                form:'mlForm',
                size:sizePar,
                onfocus:"return function(){document.getElementById('"+prefx2+suff+"').style.backgroundColor='#FFA261';"
                                         +"document.getElementById('"+prefx2+suff+"').style.borderWidth='2px';}()",
                onblur:"return function(){document.getElementById('"+prefx2+suff+"').style.backgroundColor='#FFE59A';"
                                        +"document.getElementById('"+prefx2+suff+"').style.borderWidth='0px';}()"};
};
// -- filling in
...
padLAlarm.atts={id:'padLAlarm',
                class:'lable-pad',
                style:'display:none;'
                };
labAlarm.atts={id:'labAlarm',
                class:'lable-alarm',
                innerHTML:innT[indexOf(elm,'labAlarm')]
                };
padAlarm.atts={id:'alarm-pad',
                class:'inp-pad',
                style:'display:none;'
                };
alarm.atts={id:'alarm',
...
formBels.padLMess.atts={id:'padLMess',
                  class:'lable-pad',
                  };
formBels.labMess.atts={id:'labMess',
                 class:'lable',
                 innerHTML:innT[indexOf(elm,'labMess')]
                 };
formBels.padTArMess.atts={id:'padTArMess',
                    class:'tar-messpad'
                    };
formBels.tArMess.atts={id:'tArMess',
                  class:'tar-mess',
                  required:true,
                  maxLength:'3000',
                  form:'mlForm',
                  placeholder:'Write your greate thoughts here...',
                  onfocus:"return function(){document.getElementById('tArMess').style.backgroundColor='##FFA261';"
                                           +"document.getElementById('tArMess').style.borderWidth='1px';}()",
                  onblur:"return function(){document.getElementById('tArMess').style.backgroundColor='#eea';"
                                          +"document.getElementById('tArMess').style.borderWidth='1px';}()"};
/* -- buttons  -- */
formBels.panButts.atts={id:'panButts',
                  style:"position:relative;display:block;widhth:90%;text-align:center;min-height:30px;background-color:white;"};
formBels.buttPreview.atts={id:'buttPreview',
                     class:'button',
                     onclick:"return function(){alert('preView');google.script.run.sendMailToMe('preView',document.getElementById('tArMess').value)}()",
                     innerHTML:innT[indexOf(elm,'buttPreview')]};
                     
                     
formBels.buttSend.atts={id:'buttSend',
                     class:'button',
                     onclick:"return function(){alert('send');google.script.run.sendMailToMe('mail',document.getElementById('tArMess').value)}()",
 ...
 * @param{object}selFrom - html element object of <select> tag for arrays of user e-mails
 */
...
console.log('inside getMyEMail');
    if(selFrom){
      ...
      document.getElementById('alarm').innerHTML="incorrect <select> From email-element set";
    }
    return emailsGot;
  }
/**
...
function handleError(error,elOpt){
  if(elOpt){
    ...
    document.getElementById('alarm').innerHTML='error from getMyEMail: '+error.message;
