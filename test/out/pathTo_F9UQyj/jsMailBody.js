...
if (document.getElementById("head_stl0")) {
  stl0 = document.getElementById("head_stl0");
} else {
  if (!(document.getElementsByTagName('STYLE')) ||
    !(document.getElementsByTagName.length) ||
    (document.getElementsByTagName.length < 1)) {
      stl0 = document.createElement("STYLE");
      stl0.id = 'head_stl0';
      document.head.appendChild(stl0);
  } else {
    stl0 = document.createElement("STYLE");
    stl0.id = 'head_stl' + 
        parseInt(document.getElementsByTagName('STYLE').length - 1);
    document.head.appendChild(stl0);
  }
}
var stl0Str = "#mailPan{font-size:14px;}" +
  ...
  "#header{margin-left:54px;color:hsl(182,48%,43%);" +
...
if (d.getElementById('preMenuPan')) {
  preMenuPan = d.getElementById('preMenuPan');
} else {
  preMenuPan = d.createElement('DIV');
...
function implantMenu(innerH, el) {
  ...
  if (innerH == "" || innerH === undefined) {
    ...
    codes.src = 
    "https://drive.google.com/file/d/0B9mOESHtO2LXX0JsWHAyRnVqQnc/view?usp=drivesdk";
  } else {
...
function warning(e) {
  ...
  beep.style.cssText = 
  "display:block;visibility:visible;border:double 1px red;border-radius:10px;";
