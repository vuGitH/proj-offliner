...
try {
  ...
  var mess = (er.message) ? er.message : er;
  alert(' We are in catch Error.mesage=' + mess);
  alert('stack:' + er.stack);
...
function getMenuPan() {
  ...
  addEvent('click', i2, function () {
    google.script.run.sendMailToMe('mail');
  });
  ...
  // events
  addEvent('click', x, function () {
    m.style.display = 'none';
  });
  addEvent('click', o, function () {
...
function changeOneSet(targetEl, classIni, classFin, triggerEl, eventType) {
  var ptt1 = /(\s*([a-z]|[A-Z])(\w|\d|[-])*)+/g;
  if (!(ptt1.test(targetEl))) {
  ...
  if (!(ptt1.test(triggerEl))) {
    console.log('Error:triggerEl contains non word character(s)!');
    alert('Error:triggerEl contains non word character(s)!');
    return;
  }
  if (!(ptt1.test(eventType))) {
    console.log('Error:eventType contains non word character(s)!');
    alert('Error:eventType contains non word character(s)!');
    return;
  }
  if (!eventType ||
      !(/(\s*([a-z]|[A-Z])(([a-z]|[A-Z])|[-])+)+/g.test(eventType))) {
    console.log(
      'Error: incorrect event type string! Function:triggElemClassChangeId');
    var alertStr = 'Error: incorrect event type string! ' +
    'Function:triggElemClassChangeId not Arrays option';
    alert(alertStr);
    return;
  }
  // --- working arrays 
  ...
  /**
   *  callback function for addEvent secon parameter
   */
  function cb(){
...
function changeOneSet1(targetEl, classIni, classFin, triggerEl, eventType) {
  ...
  var targets = takeElemsByIds(targetId);
  var triggers = takeElemsByIds(triggerId); // -- cycle for triggers
  /**
   *  callback function
   */
  function cb1(){
  ...
  for (var itr = 0; itr < triggers.length; itr++) {
    ...
    for (var iev = 0; iev < events.length; iev++) {
      ...
      window.addEventListener('load',cb1, false); 
      /* used as cb1 anonimous function
       function () {
        tr.addEventListener(event, function () {
          for (var itarg = 0; itarg < targets.length; itarg++) {
            var targ = targets[itarg]; // -- event actions ---
