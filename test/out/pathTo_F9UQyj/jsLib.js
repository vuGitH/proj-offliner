...
function setElPositionInsideParent(elem, xOpt, yOpt) {
  ...
  var elTop = (units == 'px') ? 
              Math.ceil((parH - elH) / 2 - yShift) + 'px' : 
              Math.ceil((((parH - elH) / 2 - yShift) / parH) * 10000) / 100 + '%';
...
function Css(selector, style) {
  ...
  this.selNew = function (select) {
    if (select != undefined && select != '') {
      this.sel = ((this.sel != undefined) && (this.sel != '')) ? 
                 this.sel + ', ' + select : select;
      this.cssStr = (((this.sel != undefined) &&
                     (this.sel != '')) &&
                     ((this.stl != undefined) && (this.stl != ''))) ?
                    this.sel + "{" + this.stl + "}" : "";
      return this.sel;
    }
  };
...
function insertCross(parent, whereOpt, colorSchemeOpt) {
  ...
  if (where == 'downRight') {
    cross.setAttribute('style',
      "position:absolute;right:4px;bottom:4px;color:" +
      xColor + "font-size:14px;margin:0px;padding:0px;" +
      "border:solid 0px;width:20px;height:20px;" +
      "cursor:pointer;font-style:normal;");
  } else if (where == 'upRight') {
    cross.setAttribute('style', '' +
      "position:absolute;right:4px;top:4px;color:" + 
      xColor + "font-size:14px;margin:0px;padding:0px;" +
  ...
  */
  // --- Cross:hover Style. Add callBackBox's Cross :hover style 
  // to css data in first <style> tag
  var styl;
  if (d.getElementsByTagName('STYLE')[0]) {
    styl = d.getElementsByTagName('STYLE')[0];
  } else {
    styl = d.createElement('STYLE');
    styl.id = 'head_stl0';
    document.head.appendChild(styl);
    styl.appendChild(d.createTextNode(''));
  }
  if (styl) {
    styl.firstChild.nodeValue += 
